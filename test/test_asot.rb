...
class AsotModelTest < Test::Unit::TestCase
  ...
  def test_calculate_rank_based_on_votes_and_uservotes
    no_1 = Asot.create(:no => 1, :airdate => Time.parse('Thu,  6 Nov 2008'), :votes => 2)
    no_2 = Asot.create(:no => 2, :airdate => Time.parse('Thu,  6 Nov 2009'), :votes => 1)
    no_3 = Asot.create(:no => 3, :airdate => Time.parse('Thu, 13 Nov 2009'), :votes => 4)
    no_4 = Asot.create(:no => 4, :airdate => Time.parse('Thu, 20 Nov 2009'), :votes => 3)
    [no_1,no_2,no_3,no_4].each{ |x| x.save! }
    ...
    no_1.vote!("127.0.0.1")
    no_1.vote!("192.168.0.1")
    no_1.vote!("192.168.0.2")
    no_1.vote!("192.168.0.3")
    assert_equal 2, no_1.votes
    assert_equal 4, no_1.uservote_count
    assert_equal 6, no_1.allvotes

    no_4.vote!("127.0.0.1")
    no_4.vote!("192.168.0.1")
    assert_equal 3, no_4.votes
    assert_equal 2, no_4.uservote_count
    assert_equal 5, no_4.allvotes

    Asot.calc_ranks
    assert_nothing_raised { no_1.rank }

    assert_equal 1, no_1.rank
    assert_equal 4, no_2.rank
    assert_equal 3, no_3.rank
    assert_equal 2, no_4.rank

    assert_equal 1, no_1.yearrank
    assert_equal 3, no_2.yearrank
    assert_equal 2, no_3.yearrank
    assert_equal 1, no_4.yearrank
  end
